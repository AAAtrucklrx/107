...
**语义提示块（Callout）映射（2026-09-15）.** 回答里的「重要限定」用 GitHub Alerts 语法单独成段
（`> [!NOTE]` 等），**沿用既有令牌、不新增色值**；每块都带**图标 + 文字标签**，
保证不依赖颜色也能读懂（WCAG 1.4.1「Use of Color」）：

| 语法 | 标签 | 令牌 | 用途 |
|---|---|---|---|
| `> [!NOTE]` | ℹ 注意 | `retrieval`（证据青） | 补充说明、样本量偏少 |
| `> [!TIP]` | ✦ 提示 | `success`（确认绿） | 正向建议 |
| `> [!IMPORTANT]` | ❗ 重要 | `primary`（操作蓝） | 需去官方系统办理 |
| `> [!WARNING]` | △ 警告 | `warning`（演示琥珀） | 接口不可用、已降级为本地缓存 |
| `> [!CAUTION]` | ⛔ 风险 | `danger`（风险红） | 演示/合成数据，可能误导判断 |

**结构化数据卡（2026-09-15）.** 15 类工具结果以表格卡直接渲染（不经 LLM 重述）。
单元格语义分级**只用于**「评分 / 均分」（≥9 高、<6 低、其余中档）与「给分」（超好·很好·好=高，
杀手·很差·差=低）；**中档不着色**，且**数值与文字本身始终是主要线索，颜色只是辅助**。
