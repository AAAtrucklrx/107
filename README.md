...
| 🌐 联网证据 | 本地不足/权威不足/问题含时效词（最新·今天·最近…）时，经隐私清洗后使用 **博查（bocha）搜索 + Crawl4AI 抓取** 保底（SearXNG 已降为备用）；达到一手权威来源或双独立可靠来源门槛后才输出确定结论；不足时展示提取内容并注明「仅供参考」，不编造 |
...
- **外部服务**: 统一身份认证（CAS）、教务系统（jw API）、青春科大 young、**DeepSeek 官网**（LLM/OCR，2026-09-04 切换）；联网搜索=博查 API（SearXNG 备用）+ Crawl4AI adapter（抓取必需）；**官方站点直采**（`scripts/collect_official_pages.py`，teach 教务 RSS 每日进"采集→审核→发布"管线，官方一手来源）
