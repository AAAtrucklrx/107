...
小蜗是一个面向中国科学技术大学师生的校园智能助手，基于 DeepSeek 官网 LLM（`api.deepseek.com/v1`，`deepseek-v4-flash`，2026-09-04 切换）构建，采用**统一 QA LangGraph** 智能体架构（意图分类 + 混合召回双通道 → LLM 自主决策循环 → 工具执行 → 统一回答），支持知识库问答与教务数据实时查询。
...
| ✅ 可核验 | 答案里的数字/日期都必须能在它披露的来源里找到；找不到就**就地标成「资料未给出」**而不是编（实测 10 题验收：10 作答 / 0 拒答，作答里"证据中找不到的数字" = 0）。语义缓存命中时**连原始来源一起还原**，不退回占位来源 |
| 🔁 反馈闭环 | 回答下方 👍/👎 → 自动转动作：「信息已过期」对该回答来源**排队复抓**、「来源有问题」生成**来源规则建议**（后台可导出 Git diff）、「有帮助」直接办结；**未登录用户的反馈同样进入审核列表**并可处理 |
| 🌐 联网证据 | **本地知识库优先**：能本地答就本地答（实测 0.9~1.3 秒），本地没把握才联网。联网走 **纯搜索（百度千帆 `references_only`，返回网页原文摘录 1000~1500 字/条）+ 自研合成 + 两道闸门**：合成前判资料是否切题；合成后**逐条核对答案里的数字/日期是否都在来源里**，不过就先改一版、仍有残留就地标「资料未给出」。公众号与全网**并行**检索，公众号确认即采用，否则以纯搜索答案优先返回；来源按「官方一手 / 官方公众号 / 独立可靠 / 未核实」四档标注 |
...
| 🎪 活动推荐 | **青春科大（第二课堂）实时数据**：对话直接查报名中活动（含地点/联系人/报名窗口，缺地点时经详情接口兜底）；每日弹窗四因子个性化推荐（紧迫度/课表空闲/个性化/热度），个性化=平台兴趣标签+行为学习+**德智体美劳模块学时均衡补短板**；token 失效自动回退本地快照 |
...
| 🧾 管理后台 | 独立 `/admin` 入口（**默认进知识审核**，工具审核在 `/admin/tools`）：进料日报、LLM 预审四项（稳定性/敏感性/重复性/相关性）、人工审核（原文·清洗稿·差异、分块、来源治理、有效期）、发布代号与一键回滚、来源规则建议计数与 Git diff 导出、反馈闭环处理；复用管理员 RBAC、CSRF、Demo/Production 命名空间、不可变审计、乐观锁与 generation 发布约束 |
...
- **外部服务**: 统一身份认证（CAS）、教务系统（jw API）、青春科大 young、**DeepSeek 官网**（LLM/OCR，2026-09-04 切换）；联网搜索=百度千帆 API（自带权威分；bocha/SearXNG 可切换）+ Crawl4AI adapter（抓取必需）；**官方站点直采**（`scripts/collect_official_pages.py`，teach 教务 RSS 每日进"采集→审核→发布"管线，官方一手来源）
...
| `python -m pytest tests/web -q` | 269 passed（Web API、认证/权限、通知驱动 SSE、SSRF、结构化证据、审核队列与 generation） |
