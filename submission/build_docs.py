...
BRIEF_SECTIONS: list[tuple[str, list]] = [
    ...
    ("三、解决的问题", [
        ...
        "信息可信度：回答遵循证据门槛——确定结论须有官方一手来源，或一个可靠来源加一个独立佐证；"
        "证据不足时如实说明并列出已检索内容（标注仅供参考），绝不编造。所有降级/缓存数据都带来源标识（实时数据/本地缓存/第三方工具）。",
    ...
    ("四、核心功能", [
        ...
        "联网证据通道：本地资料不足或问题要求最新信息时，经隐私清洗后联网检索（百度千帆搜索 API，自带权威分排序），"
        "并按证据门槛核验后才输出确定结论；科大相关问题并行检索微信公众号官方号与全网，结果合并核验。",
        ...
        "自成长知识库：联网问答中验证可靠的公开证据会自动进入审核队列（异步、不阻塞回答），"
        "经人工逐块审核后发布为新知识版本并重建索引——下次同类问题直接本地命中，不再联网；"
        "已发布来源由后台定期复抓，内容变更自动重新进审。小蜗用得越多，知识库越丰富、越新鲜。",
    ]),
    ("五、使用的模型及 API 调用方式", [
        ...
        "联网搜索：百度千帆搜索 API（自带 authority_score 权威分与摘要，国内直连，搜索源可配置切换）；"
    ...
    ("六、创新点", [
        "证据门槛的可信回答：将\"AI 不编造\"做成机制而非提示词——确定结论须过官方一手来源或\"可靠来源+独立佐证\"门槛，"
        ...
        "AI 输出可治理且知识自成长：联网验证可靠的公开证据自动进入\"采集→清洗→逐块审核→generation 发布→可回滚\""
        "流水线，审核通过即入库重建索引——回答一次，下次本地直接命中；后台定期复抓已发布来源，内容变更"
        "自动重新进审，知识保鲜。原始快照不可变，发布索引与回滚约束保证知识库变更全程可追溯。",
        "严格的身份与隐私边界：专业/年级只取已验证档案、匿名选择不继承、demo 与生产数据永久隔离；"
        "联网抓取不携带任何校内凭证，SSRF 防护覆盖内网/回环/云元数据/重定向。",
        "安全素养与责任披露：开发校园二课活动推荐板块时，我们对青春科大平台进行了接口测试，"
        "发现一处水平越权风险——我们未利用、未扩散，当日通过官方渠道向校方老师提交了完整的安全报告；"
        "校方两天内确认修复。负责任披露与对校内系统的尊重，与我们在小蜗内做的每一项防护是同一件事。",
    ...
    ("七、运行与部署", [
        ...
        "测试基线：pytest tests/web 229 项全绿；工具/节点/安全等 verify 系列脚本 260+ 断言通过。",
    ]),
]
...
DESIGN_SECTIONS: list[tuple[str, list]] = [
    ("一、评分项映射", [
        ...
        "另附 §六 开发插曲（负责任披露的真实经历）与 §七 运行环境与演示。"
        "评审参考的评分项对应章节与核心证据如下，便于快速定位：",
        ("table",
         ["评分维度", "分项标准", "对应章节", "核心证据"],
         [
             ["创新性", "场景创新", "§2.1", "信息可信度与跨系统整合——未被现有工具覆盖的需求；知识自成长闭环（联网→审核→入库）"],
             ...
             ["技术难度", "调试与优化能力", "§5.2 图 4", "实测优化数据表、三层降级、229 项回归测试"],
             ["完成度", "作品完整性", "§6/§7", "生产部署 + 公网可访问 + 测试基线全绿；负责任披露插曲（§6）"],
             ["完成度", "设计文档与材料质量", "§7", "仓库文档体系 + 本文档架构图与模型调用说明"],
             ["完成度", "演示效果", "§7", "公网演示环境 + ≤5 分钟演示视频"],
         ]),
    ]),
    ("二、设计思路", [
    ...
    ("三、技术架构", [
        ...
        "进程内通知唤醒 + 1 秒轮询兜底（支持 Last-Event-ID 断线续传）；联网搜索为百度千帆 API（自带权威分排序，国内直连）+ "
    ...
    ("四、功能模块", [
        ...
        ("table",
         ...
         [
             ...
             ["时效信息", "\"最新的选课通知\"", "联网检索（公众号+全网并行，结果合并）→ 证据门槛核验 → 附来源"],
        ...
        "管理后台（知识审核/工具治理/审计）。",
        "自成长知识库（联网→审核→入库闭环）：联网问答中验证可靠的公开证据自动进入审核队列"
        "（异步、不阻塞回答；demo 与生产命名空间隔离），人工逐块审核后发布为新 generation 并重建双索引，"
        "下次同类问题直接本地命中；已发布来源由后台定期复抓，内容变更自动重新进审；"
        "官方站点直采与教务 RSS 每日进线为另一路稳定供给。知识库从 0 到 80 篇、从静态到持续成长，"
        "全部变更有不可变快照与回滚约束。",
        ...
        "工程实践可参考：229 项 Web 回归测试、260+ 工具断言、LLM 一致性金标评测的完整测试体系，"
        "对同类课程项目/比赛项目有直接复用价值。",
    ]),
    ("五、技术难度", [
        ...
        "搜索结果重排（信任级主序 + 百度 authority_score 权威分副序，官方源优先抓取）；"
        "证据门槛（官方一手来源，或可靠来源+独立佐证，才输出确定结论）；"
        "知识治理流水线（采集→LLM 清洗→逐块审核→generation 发布→双索引重建，不可变快照可回滚）；"
        "三路供给汇聚：联网问答自动进审（见 §4.2 自成长知识库）、官方站点直采与教务 RSS 每日进线、"
        "已发布来源定期复抓（refetch，内容变更自动重新进审）——检索质量随使用持续提升。",
        ...
        ("table",
         ...
         [
             ...
             ["回归测试", "—", "pytest 229 项 / 前端 22 项 / verify 系列 260+ 断言 / LLM 金标 12/12"],
         ]),
        ...
        "审计不可变、密钥经 .env 注入不入 git。安全意识也曾延伸到校外——开发中发现青春科大平台一处越权风险"
        "并负责任披露（完整经过见 §6 开发插曲）。",
    ]),
    ("六、开发插曲：一次负责任的漏洞披露", [
        "这是一个开发过程中的真实小插曲，我们把它如实写进文档，因为它体现了团队对待安全问题的态度。",
        ("h3", "背景：为做二课推荐，对接青春科大平台"),
        "小蜗的活动推荐板块需要对接青春科大\"智慧团学\"平台。为了确认接口的行为边界，"
        "团队对相关接口做了常规的功能测试。",
        ("h3", "发现：一处水平越权风险"),
        "测试中我们注意到 personal 接口按用户标识返回数据，且未见严格的属主校验——这是典型的水平越权"
        "（IDOR）风险：理论上任何一个登录用户都可能读到其他同学的个人数据。",
        ("h3", "处置：未利用、未扩散，当日负责任披露"),
        "确认风险后，我们立即停止进一步测试、不留存任何他人数据，当日（2026-08-23）整理了包含"
        "复现路径、风险评级与修复建议的完整安全报告，通过官方渠道提交给平台负责老师，"
        "并主动说明了团队身份（107 杯比赛团队）。",
        ("h3", "结果：校方两天内确认修复"),
        "平台老师次日回复\"非常重要，技术人员上班后将处理\"，8 月 25 日确认：\"该漏洞已修复，"
        "再次感谢你们的热心反馈\"。从发现到修复闭环，不到两天。",
        ("img", "6-disclosure-emails.png",
         "图 5  校方邮件回复：首轮响应安排处理（左）与 8 月 25 日修复确认（右）"),
        ("h3", "插曲的启示"),
        "安全问题不挑场合——为自己做功能时发现的问题，同样要以对待生产事故的标准来处理。"
        "这次经历也直接影响了小蜗的设计：§2.2 的身份与数据边界取舍、全链路的越权与隐私防护，"
        "都源于团队对\"数据属主\"这件事的切身敏感。我们相信，一个值得信赖的校园智能体，"
        "不仅要答得对，还要守得住边界。",
    ]),
    ("七、运行环境与演示", [
        ...
        "测试基线：pytest tests/web 229 项 / 前端 vitest 22 项 / verify 系列 260+ 断言 / LLM 一致性金标 12/12。",
        "文档体系：仓库维护 README（快速上手）、PRODUCT/DESIGN（产品与设计系统事实）、tool-specs（30+ 工具详细规格）、"
        "Web 技术规格（Q1–Q80 已确认契约）、部署规格与运维记录、开发日志；模型调用说明见作品简介第五节"
        "（deepseek-v4-flash/qwen3-embedding/unlimited-ocr/百度千帆搜索的用途与调用方式）。",
