...
设计文档按评审评分标准四维度（创新性/实用性/技术难度/完成度）组织，
首节给出评分项→章节→证据映射表；嵌入 Mermaid 图（diagrams/*.png）、
Prompt 工程节选与实测数据表。
...
BRIEF_SECTIONS: list[tuple[str, list]] = [
    ...
    ("七、运行与部署", [
        ...
        "测试基线：pytest tests/web 223 项全绿；工具/节点/安全等 verify 系列脚本 260+ 断言通过。",
    ]),
]
...
DESIGN_SECTIONS: list[tuple[str, list]] = [
    ("一、评分项映射", [
        "本文档按评审参考的四维度组织章节。每一项评分标准对应的章节与核心证据如下，"
        "便于快速定位：",
        ("table",
         ["评分维度", "分项标准", "对应章节", "核心证据"],
         [
             ["创新性", "场景创新", "§2.1", "信息可信度与跨系统整合——未被现有工具覆盖的需求"],
             ["创新性", "交互与体验创新", "§2.2", "全链路流式（首字 3.1s）、结构化数据卡、思考过程可视、降级透明"],
             ["创新性", "多智能体协作创新", "§2.3 图 2", "五角色协作的 LangGraph 统一图 + 规则/LLM 混合编排 + 并行工具"],
             ["实用性", "场景真实性与问题匹配度", "§3.1", "四类真实校内数据源；教学/生活/服务三场景全覆盖"],
             ["实用性", "功能完整性与解决方案有效性", "§3.2", "十类功能模块全部实现并在生产环境运行"],
             ["实用性", "可扩展性与推广价值", "§3.3", "生态工具协议、配置驱动服务、架构可迁移其他高校"],
             ["技术难度", "大模型应用深度", "§4.1 图 3", "混合检索/查询改写/语义缓存 + 22 条决策规则 Prompt + 证据门槛"],
             ["技术难度", "Workflow 与系统设计", "§4.2 图 1", "五层架构、模块划分清晰、generation 版本化"],
             ["技术难度", "调试与优化能力", "§4.3 图 4", "实测优化数据表、三层降级、223 项回归测试"],
             ["完成度", "作品完整性", "§5.1", "生产部署 + 公网可访问 + 测试基线全绿"],
             ["完成度", "设计文档与材料质量", "§5.2", "仓库文档体系 + 本文档架构图与模型调用说明"],
             ["完成度", "演示效果", "§5.3", "公网演示环境 + ≤5 分钟演示视频"],
         ]),
    ]),
    ("二、创新性", [
        ("h3", "2.1 场景创新：切入\"信息可信度\"这一未被关注的需求"),
        "调研发现（备赛阶段完整梳理了科大 8 个官方平台、12 个学生工具与历届获奖项目）：现有工具全部是"
        "\"人找信息\"的被动模式——导航站聚合链接、门户 App 展示信息，没有一个解决两个更根本的问题："
        "(1) 信息分散在十余个系统，跨系统任务（如\"选课要同时看培养方案+评课+课表冲突\"）只能人工来回比对；"
        "(2) 群聊截图与过时通知泛滥，学生无从核验一条教务信息是真是假、是不是最新。",
        "小蜗把这两个\"没人做\"的需求作为核心切入点：跨系统任务一句话完成 + 每个答案可核验来源。"
        "\"可信度\"作为一等公民贯穿全系统——证据门槛、来源标识、降级标注，而非事后补丁。",
        ("h3", "2.2 交互与体验创新"),
        ...
        "编排方式是本项目的核心探索：**规则与 LLM 自主决策的混合编排**——高置信意图（成绩/课表/培养方案等）"
        "走确定性路由完全跳过决策 LLM（省 20–60 秒往返且行为可预期），长尾意图交给带规则约束的 LLM 自主决策"
        "（≤4 轮循环，每轮工具表按意图裁剪）。纯规则系统无法覆盖长尾，纯 LLM 决策又慢且不可控，"
        "两层结合兼得可靠性与灵活性。",
    ]),
    ("三、实用性", [
        ("h3", "3.1 场景真实性与问题匹配度"),
        "全部数据源均为校内真实数据：综合教务 jw API（成绩/课表/考试）、青春科大 young 平台（第二课堂活动）、"
        "评课社区 icourse.club 公开数据（5667 个课程页 / 4.4 万条评论）、教务处官网与官方公众号"
        "（80 篇文档经采集审核入库）。覆盖评分标准点名的教学（选课/成绩/课表）、生活（活动/日程/办事）、"
        "服务（官方入口/校园工具）三类场景。",
        "推荐语义经真实用户确认打磨：课程范围是硬条件，兴趣/工作量/教师/学期默认软排序，"
        "仅\"只要/必须\"升级为硬过滤——这来自多轮真实选课咨询的语义校准，而非想当然的过滤设计。",
        ("h3", "3.2 功能完整性与解决方案有效性"),
        ("table",
         ["场景", "用户问题示例", "小蜗的解决路径"],
         [
        ...
        "工程实践可参考：223 项 Web 回归测试、260+ 工具断言、LLM 一致性金标评测的完整测试体系，"
        "以及 37 天从零到生产部署的全过程文档，对同类课程项目/比赛项目有直接复用价值。",
    ]),
    ("四、技术难度", [
        ("h3", "4.1 大模型应用深度（Prompt 工程 + RAG）"),
        ...
        ("table",
         ...
         [
             ...
             ["回归测试", "—", "pytest 223 项 / 前端 22 项 / verify 系列 260+ 断言 / LLM 金标 12/12"],
         ]),
        "可靠性同样体系化：LLM 熔断窗（仅连接级失败开窗，读超时不动窗）；数据降级带来源标注；"
        "并发有界队列与 503 RUN_BUSY 背压；prompt 注入快照测试（qa_consistency 断言体系）。",
    ]),
    ("五、完成度", [
        ("h3", "5.1 作品完整性：功能完整实现并正常运行"),
        "系统已完整部署于 Linux 生产服务器（Ubuntu 24.04，Web:8000 公网 8850 / 审核发布 worker 常驻 / "
        "Streamlit 回退入口），公网地址 http://114.214.241.119:8850 当前可访问，readiness 健康端点四项哨兵"
        "（数据库/审核库/批准索引/检索质量）全绿。",
        "全部十类功能模块（问答/证据/课业/选课/培养方案/日程/活动/官方入口/校园工具/管理后台）"
        "均实现并接入真实数据源；全队 3 人 37 天从零完成。",
        ("h3", "5.2 设计文档与材料质量"),
        "仓库内维护完整文档体系：README（快速上手）、PRODUCT/DESIGN（产品与设计系统事实）、"
        "tool-specs（30+ 工具详细规格）、Web 技术规格（Q1–Q80 已确认契约）、部署规格与运维记录、"
        "开发日志（每个 finding 的修复过程）。本文档含架构图（图 1/2/3/4）与模型调用说明（见作品简介第五节："
        "deepseek-v4-flash/qwen3-embedding/unlimited-ocr/博查搜索的用途与调用方式）。",
        ("h3", "5.3 演示效果"),
        "≤5 分钟演示视频按\"开场工作流总览 → 问答与来源核验 → 个人数据直连 → 选课顾问 → 学业工作区 → "
        "联网证据门槛 → 校园服务与知识治理\"编排，覆盖全部核心运行流程；公网演示环境可供评委实际操作验证"
        "（合成演示身份，个人数据明确标识为演示数据）。",
    ]),
]
