...
>
> ⚠️ **2026-09-17 更新（优先于下文所有快照数值）**：HEAD = `origin/main` = **`eca69af`**（服务器与远端一致）；
> 测试基线 `pytest tests/web` **470 passed** / `vitest run` **37 passed**；8 服务 RUNNING、readiness 六项全绿。
> 联网问答已定稿为 **纯搜索 + 自研合成 + 两道闸门（含就地对冲）**，`XIAOWO_WEB_ANSWER_MODE=pure`，且**纯搜索先答、公众号降为兜底**；
> 反馈闭环打通（未登录用户反馈可见可处理）；管理后台 `/admin` **默认进知识审核**。
> 文档该信哪份见 `docs/文档索引与时效.md`；最新状态见 `docs/接手日志.md` 顶部 ⚡ 节。
...
> **部署已完成**（competition+demo @ `http://114.214.241.119:8850`，origin 单值校验），HEAD `debef74`，数据资产（评课库/主库/审核库/chroma/young 快照）全部就位；
