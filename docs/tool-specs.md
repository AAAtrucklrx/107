...
```python
{
    ...
    # 同名多页披露（2026-09-22）：一门课在评课社区常有多页（每位老师/班次一页），
    # 头号分数只是一页的分数，必须让同学知道来处与范围
    "same_name_pages": 30,          # 同名页数（样本>0）
    "same_name_scored_pages": 23,   # 其中样本≥3 的页数
    "score_range": "1.5~10.0",      # 各页（样本≥3）分数区间
    "page_note": "本分数只代表「量子物理」在评课社区的一个页面（涂涛，62 条）…",
...
2. 硬条件：keywords / course_scope；兴趣、工作量、教师、目标学期仅在用户说“只要/必须”时升级为硬过滤。
   ...
   - 关键词先剥掉尾部角色词（“老师/教师/教授/班级/班/的”）再查库校验：
     「线性代数（B1）老师推荐」→ keywords=`["线性代数（B1）"]`（2026-09-22 修复，此前整词被丢→“0 门”）。
...
9. 指定课程直查（`source="exact_course"`，列该课全部班级，不受 max_results 截断）的评分呈现纪律
   （2026-09-22 修用户实测「未知老师 + 评分极低」）：
   - **0 条样本**的班级不入表 → 计入 `unrated_class_count` + limitations（“没有任何评课数据”≠“0 分”）；
   - **评课页未标注教师且样本 < 3 条**的不入表 → 计入 `unlabeled_class_count` + limitations
     （此前渲染成「未知 | 10.0分·1条」）；
   - **样本 < 3 条**的班级排在最后，并带 `sample_note`（“样本量少（仅 N 条），参考价值有限”）；
   - 排序按样本量降序，真实大样本班级在表首（此前两条 1 条样本的裸课名排在 92 条样本的班级前面）。
```

**数据卡与「点击追问」（2026-09-22）**：
- `data.table` 推给前端的数据卡可带两个可选字段：`row_questions`（与 `rows` 一一对应的追问问句，
  如「量子物理有哪些老师？」）与 `actions`（卡片级快捷按钮，如「量子物理 的评论原文」）。
- 前端点击该行/按钮**只把问句填进输入框并聚焦，不自动发送**，同学可改完再发。
- 生成规则（**每一行都可问**）：
  - 评课对比卡（一行=一个班）：单个老师名（2~4 汉字）→「<课> <老师> 老师怎么样？」；合教组合 →
    「<课> <甲、乙> 合教的班怎么样？」；评课页未标注老师 →「<课> 未标注老师的那个班怎么样？」；
    其它异常名 →「<课> <原名> 这个班怎么样？」。
  - 课程推荐 / 搜索结果卡（一行=一门课）：带老师 → 按该老师问（同上）；不带老师 →「<课>怎么样？」。
  - **不得**统一问「<课>有哪些老师？」（2026-09-22 用户实测「电磁学C 点击后还是有哪些老师」后改）。
  - 各问法均已真机验证能答到"这个班"或"这门课"。
- `analyze_teacher` 课程模式额外返回 `community_url` / `same_name_pages` / `score_range` / `page_note`，
  摘要里同时给出评课页链接与下钻指引；`_build_tool_summary` 对课程类结果统一追加
  「下钻指引：想按老师看可问『<课>有哪些老师』、想看评论原文可问『<课>的评论』」。
...
**2026-09-05 增补（事件面扩展）**：
- **`data.table`**：结构化数据卡（成绩/课表/考试/选课/活动/空教室/培养方案/日周视图/课程搜索/课程推荐/评课对比/课程对比 **15 类**工具结果）——**工具完成即推**（先于正文，前端先行渲染）；内容 `{title, columns, rows, source_tool}`；服务端对同一张卡按**内容指纹**（sha256）抑制重复推送（2026-09-15），前端按 `title|tool|JSON(rows)` 覆盖去重。
