> ⚠️ **时效标注（2026-09-17 核对）**：本文是**当时的设计/快照**，其中以下内容已被取代 —— 联网证据已定稿为「纯搜索 + 自研合成 + 相关性闸门 + 可核验闸门（含就地对冲）」，且**纯搜索先答、公众号降为兜底**；文中「百度智能搜索生成」不再是默认路径。
> 当前实现一律以 [`docs/接手日志.md`](接手日志.md) 顶部 ⚡ 节 + [`docs/文档索引与时效.md`](文档索引与时效.md) 为准。
...
- **校园服务**：服务入口与公开活动使用独立搜索/筛选状态；入口包含配置驱动的精选区与分类目录，活动保留时间列表。
...
| 官方入口与青春科大公开活动 | “校园服务”独立入口/活动搜索 + `/campus/*` | 官方链接与活动回退旧回归保留；无数据时不编造 |
...
| 浅深主题与移动端 | 定制“小蜗”字标、桌面 216px 索引脊、平板 72px 图标轨、移动底栏 | 1440 / 1024 / 768 / 390 / 320px，无整页横向溢出、控制台错误或低于排版/触控下限的控件 |
...
SSE `id` 是单个 run 内从 1 开始单调递增的 sequence；`Last-Event-ID` 也只在该 run 内解释。游标早于当前保留窗口时返回 HTTP 410 `EVENT_CURSOR_EXPIRED`。终态事件与 run 终态必须在同一事务提交；事件流观察到 terminal 状态时仍需按最后游标补读一次，再关闭连接，不能遗漏同事务写入的 `answer.completed`、`run.cancelled` 或 `run.failed`。登出、会话轮换或 demo 登录会取消旧会话仍在运行的 run，旧 principal 此后不能读取或取消它们；已保存的终态事件按原 TTL 清理。
