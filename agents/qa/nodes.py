...
- **重要限定必须用语义提示块单独成段**（GitHub Alerts 语法）：`> [!NOTE]` 注意（补充说明、样本量偏少）、`> [!WARNING]` 警告（接口不可用、已降级为本地缓存）、`> [!CAUTION]` 风险（演示或合成数据、可能误导判断）、`> [!IMPORTANT]` 重要（需去官方系统办理）。**一块只写一件事**、正文其余部分不要滥用；块内文字必须自己说清含义（颜色只是辅助，不得只靠颜色传达信息）
- {structured_note}
...
def _build_tool_summary(results: list[dict]) -> str:
    ...
    for r in results:
        ...
        if r.get("status") == "error":
            ...
            if exact:
                lines.append(
                    f"【指定课程直查·共{len(req)}个班级，全部为该课程全部班级（非培养方案推荐），"
                    f"回答时必须逐班列全所有 {len(req)} 个班级，不得遗漏、合并同老师班级或只挑高分班；"
                    f"不得使用必修/选修/方案学期等培养方案措辞】"
                )
                _dump(req)
            elif req:
