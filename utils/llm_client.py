...
def llm_content(response: Any, *, strip: bool = True) -> str:
    """提取模型回复文本。

    Args:
        strip: **流式增量必须传 False。**
            默认 True 去掉首尾空白，用在「整篇回复」上没问题；但逐 chunk 调用时，
            任何位于 chunk 边界（或被单独成块）的**换行都会被削掉**。
            实测（2026-09-15）：模型输出的 7 个换行经逐 chunk strip 后剩 0 个，
            导致 Markdown 表格 / 列表 / 引用全部塌成一行——Web(SSE) 版正文格式崩坏，
            而非流式路径（Streamlit/整篇 invoke）不受影响，故长期未被发现。

    DeepSeek 官网 V4 系列偶发 content 为空、推理内容落在
    additional_kwargs.reasoning_content——作为最后兜底使用
    （正常时与 response.content 一致，不改变现有行为）。
    """
    if response is None:
        return ""
    try:
        ...
        if strip:
            ...
            # 流式增量：**原样返回**（含纯空白块），交由调用方拼接
            return raw
        kwargs = getattr(response, "additional_kwargs", None) or {}
        fallback = str(kwargs.get("reasoning_content") or "")
